...
def parser_data():
    ...
    parser.add_argument(
        "--data_dir", default="../data", type=str, help="The path of the data directory"
    )
    parser.add_argument(
        "--ckpt_dir",
        default="results",
...
def get_wandb_running_name(args):
    ...
    if seed != 42:
        print(
            f"GAN_{backbone}_latent_dim_{latent_dim}_generator_hidden_dim_{generator_hidden_dim}_discriminator_hidden_dim_{discriminator_hidden_dim}_seed_{seed}"
        )
        wandb_run_name = f"{backbone}_{latent_dim}_{generator_hidden_dim}_{discriminator_hidden_dim}_{seed}"
    else:
        print(
            f"GAN_{backbone}_latent_dim_{latent_dim}_generator_hidden_dim_{generator_hidden_dim}_discriminator_hidden_dim_{discriminator_hidden_dim}"
        )
        wandb_run_name = (
            f"{backbone}_{latent_dim}_{generator_hidden_dim}_{discriminator_hidden_dim}"
        )
...
if __name__ == "__main__":
    ...
    if using_wandb:
        ...
        wandb.init(project="test", entity="eren-zhao", name=wandb_run_name)
        wandb.config = {
            "latent_dim": args.latent_dim,
            "generator_hidden_dim": args.generator_hidden_dim,
            "discriminator_hidden_dim": args.discriminator_hidden_dim,
    ...
    netG, netD = GAN.get_model(
        1, args.latent_dim, args.generator_hidden_dim, args.backbone, device
    )
    if args.do_train:
        optimG = optim.Adam(
            netG.parameters(), lr=args.learning_rate, betas=(args.beta1, 0.999)
        )
        optimD = optim.Adam(
            netD.parameters(), lr=args.learning_rate, betas=(args.beta1, 0.999)
        )
        trainer = Trainer(device, netG, netD, optimG, optimD, dataset, args.ckpt_dir)
        trainer.train(
            args.num_training_steps, args.logging_steps, args.saving_steps, using_wandb
        )
        ...
        save_image(
            make_grid(
                netG(torch.randn(size=(50, args.latent_dim, 1, 1), device=device)),
                nrow=10,
                normalize=True,
                value_range=(-1, 1),
            ),
            os.path.join(args.ckpt_dir, "Mode Collapse Problem.png"),
        )
        for pair in range(5):
            #! 做线性插值实验
            z1 = torch.randn(size=(args.latent_dim, 1, 1), device=device)
            z2 = torch.randn(size=(args.latent_dim, 1, 1), device=device)
            experiments = [z1 + i / 10 * (z2 - z1) for i in range(10)]
            experiments = torch.stack(experiments)
            save_image(
                make_grid(
                    netG(experiments), nrow=5, normalize=True, value_range=(-1, 1)
                ),
                os.path.join(args.ckpt_dir, f"interpolate_{pair}.png"),
            )

    steps = [each for each in os.listdir(args.ckpt_dir) if not each.endswith(".png")]
    restore_ckpt_path = os.path.join(
        args.ckpt_dir, str(max(int(step) for step in steps))
    )
    ...
    with torch.no_grad():
        ...
        while samples is None or samples.size(0) < num_samples:
            imgs = netG.forward(
                torch.randn(args.batch_size, netG.latent_dim, 1, 1, device=device)
            )
            if samples is None:
                samples = imgs
            else:
