...
class TfmrAttention(nn.Module):
    ...
    def _merge_heads(self, tensor : torch.Tensor, num_heads, attn_head_size):
        # TODO START
...
class TfmrLMHeadModel(nn.Module):
    ...
    def inference(self, device, PAD_ID, batch_size, maxlen, decode_strategy, temperature, top_p=1.0, top_k=50267):
        ...
        allgen = []
        with torch.no_grad():
            for i in tqdm(range(0, int(5000/batch_size)+1)):
                ...
                for _ in range(maxlen):
                    ...
                    if decode_strategy == "top-p":
                        ...
                        logits[logits < logits.topk(top_k, dim = 1).values[:, -1].unsqueeze(dim = -1)] = -1E10
                        # TODO END
