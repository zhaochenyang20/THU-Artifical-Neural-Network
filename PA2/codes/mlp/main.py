...
parser.add_argument('--data_dir', type=str, default='/home/helehan/桌面/cifar-10_data',
                    help='Data directory. Default: data/cifar-10_data')
...
def train_epoch(model, X, y, optimizer):  # Training Process
    ...
    while st < len(X) and ed <= len(X):
        optimizer.zero_grad()
        X_batch, y_batch = torch.from_numpy(X[st:ed]).to(
            device), torch.from_numpy(y[st:ed]).to(device)
